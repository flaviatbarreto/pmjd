...
(function($) {
	 ...
	 $.lofSidernews = function( obj, settings ){
		...
		this.maxWidth  = this.settings.mainWidth || 740;
		
		

		this.wrapper = $( obj ).find( this.settings.wapperSelector );	
		var wrapOuter = $('<div class="sliders-wrapper"></div>').width( 740 );
		...
		if( this.settings.direction == 'opacity') {
			...
			this.wrapper.css({'left':'-'+this.currentNo*this.maxSize+'px', 'width': 740 * this.slides.length } );
		}
	 ...
	 $.lofSidernews.fn.extend({
		...
		__getDirectionMode:function(){
			switch( this.settings.direction ){
				...
				default: this.maxSize=740; return ['left','width'];
			}
		},
